...
if(!exists("holidays")){
  holidays<-fread("spanish_holidays.csv")
...
cont_pre<-fread("Contacts_Pre_2017.csv")
contracts_end<-fread("Contracts_End.csv")
contracts_new<-fread("Contracts_New.csv")
res<-fread("Resolution_Pre_2017.csv"
  ,select=c("Date","Category","Subject","Resolution"))
res_test<-fread("Resolution2017.csv")
res[,ID:=.I]
...
resPredict[1:10,]
summary(resPredict)
fwrite(resPredict[,.(ID,Resolution=pmax(0,Resolution))][order(ID)],"Resolution2.csv")


rmse(trainR$Resolution,0)
rmse(trainR$Resolution,mean(trainR$Resolution))
trainR[,tgt_cat_subj_wkday:=mean(Resolution),.(Category,Subject,wkday)]
rmse(trainR$Resolution,trainR$tgt_cat_subj_wkday)

trainR[order(-Resolution)][1:20]
cont_pre[order(-Contacts)][1:20]
(5571-3000)^2
